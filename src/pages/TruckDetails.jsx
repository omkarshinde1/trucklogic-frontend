...
// Remember: Create and import this component file as structured earlier
...
const TruckDetails = () => {
    ...
    // 📅 DYNAMIC LOGICAL CONTROLS STATES FOR AUDIT REPORTING
    ...
    // Function to filter trips based on dates or preset ranges
    const getFilteredTrips = (tripsList, filter, start, end) => {
        const now = new Date();
    ...
    // Execute dynamic analytical pipeline filtering on render time
    const filteredTripsList = getFilteredTrips(trips, filterType, customStart, customEnd);
    const reportTitleText = filterType === 'all' ? 'Full Lifetime Statement' : `${filterType} ledger audit`;
    ...
    return (
        <div style={{
            ...
            <div style={{ flex: 1, padding: isMobile ? '20px' : '40px', boxSizing: 'border-box', overflowY: 'auto' }}>
                ...
                {/* 📊 ADVANCED INTERACTIVE STATEMENTS DOWNLOADER PANEL */}
                <div style={{ backgroundColor: '#fff', padding: '24px', borderRadius: '16px', marginBottom: '30px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.01)' }}>
                    <h4 style={{ margin: '0 0 15px 0', fontSize: '15px', fontWeight: '700', color: '#0f172a', letterSpacing: '-0.2px' }}>📊 Advanced Statements Downloader Panel</h4>
                    
                    <div style={{ display: 'flex', flexDirection: isSmallMobile ? 'column' : 'row', flexWrap: 'wrap', gap: '15px', alignItems: isSmallMobile ? 'stretch' : 'center' }}>
                        <select 
                            value={filterType} 
                            onChange={(e) => setFilterType(e.target.value)}
                            style={{ padding: '11px 14px', borderRadius: '8px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', fontWeight: '600', color: '#334155', outline: 'none', fontSize: '14px' }}
                        >
                            <option value="all">📁 All Lifetime Logs</option>
                            <option value="monthly">📅 This Month's Overview</option>
                            <option value="yearly">🗓️ This Year's Ledger</option>
                            <option value="custom">🛠️ Custom Date Range</option>
                        </select>

                        {filterType === 'custom' && (
                            <div style={{ display: 'flex', flexDirection: isSmallMobile ? 'column' : 'row', alignItems: isSmallMobile ? 'stretch' : 'center', gap: '10px' }}>
                                <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)} style={{ padding: '10px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '14px', color: '#334155' }} />
                                <span style={{ fontWeight: '600', color: '#64748b', textAlign: 'center' }}>to</span>
                                <input type="date" value={customEnd} onChange={(e) => setCustomEnd(e.target.value)} style={{ padding: '10px 12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '14px', color: '#334155' }} />
                            </div>
                        )}

                        <DownloadReportButton 
                            filteredTrips={filteredTripsList} 
                            truckInfo={truckInfo} 
                ...
                <div style={{ marginTop: '25px' }}>
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <span style={{ backgroundColor: '#e0f2fe', color: '#0369a1', fontSize: '12px', fontWeight: '700', padding: '2px 8px', borderRadius: '20px' }}>{filteredTripsList.length} Journeys</span>
                        </div>
                    ...
                    {filteredTripsList.length === 0 ? (
                        <div style={{ textAlign: 'center', backgroundColor: '#fff', padding: '50px', borderRadius: '16px', color: '#64748b', boxShadow: '0 1px 3px rgba(0,0,0,0.01)' }}>
                            ...
                            <p style={{ margin: 0, fontSize: '16px', fontWeight: '500' }}>No active transit manifests matching selected statement configurations.</p>
                        </div>
                    ) : (
                        <div style={{
                            display: 'grid',
